            ...
            <>
              ...
              <div className="form-group">
                ...
                <div style={{position:'relative'}}>
                  <input className="form-input" type={mostrarPass?'text':'password'} value={form.password} onChange={e=>set('password',e.target.value)} placeholder="mínimo 6 caracteres" style={{paddingRight:'44px'}} />
                  <span onClick={()=>setMostrarPass(m=>!m)} style={{position:'absolute',right:'14px',top:'50%',transform:'translateY(-50%)',cursor:'pointer',fontSize:'16px',color:'var(--texto-muted)',userSelect:'none'}}>
                    {mostrarPass ? '🙈' : '👁️'}
                  </span>
                </div>
              </div>
              <div className="form-group">
                ...
                <div style={{position:'relative'}}>
                  <input className="form-input" type={mostrarPass?'text':'password'} value={confirmarPass} onChange={e=>setConfirmarPass(e.target.value)} placeholder="repita a password" style={{paddingRight:'44px'}} />
                </div>
              </div>
              <div className="form-group">
              ...
              <button className="btn btn-primary btn-full"
                onClick={()=>{ if(!form.nome||!form.email||!form.password||!form.passwordConfirmar||!form.telemovel||!form.nif||!form.morada||!form.codigoPostal||!form.localidade){setErro('Preencha todos os campos obrigatórios.');return;} if(form.password!==form.passwordConfirmar){setErro('As passwords não coincidem.');return;} if(form.password.length<6){setErro('A password deve ter pelo menos 6 caracteres.');return;} if(form.password.length<6){setErro('A password deve ter pelo menos 6 caracteres.');return;} if(form.password!==confirmarPass){setErro('As passwords não coincidem.');return;} setErro('');setPasso(2) }}>
                Continuar
              </button>
